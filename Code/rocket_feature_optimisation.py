...
def rocket_training_run(feature_list):
    ...
    joblib.dump(feature_scaler, './rocket_feature_selection/rocket_feature_scaler.joblib')
    ...
    #X_train_rocket = pd.concat([pd.DataFrame(X_train_rocket), extra_features[0]], axis=1)
    ...
    adjust_ratio = True
    logistic_clf = models.train_logistic_regression([np.concatenate((X_train_rocket, X_test_rocket)), np.concatenate((y_train, y_val)), X_val_rocket, y_val], search_best_params=False, verbose=0)
    ...
    #y2_sols = list(range(862, 1441))
    #raw_predictions, total_pdrops_excluded, total_below_thresh = helpers.predict(feature_list=feature_list, test_sols=y2_sols, best_clf=model, model_type=model_type, feature_scaler=feature_scaler, pressuredrops=pressuredrops, power_threshold=power_threshold, sample_interval=sample_interval, sample_size=window_size, features=features)
    #result = helpers.evaluate_predictions(raw_predictions, y2_sols, pressuredrops, match_threshold=best_match_threshold, sample_size=window_size)
    #positive_predictions = result['predictions']
    adjust_ratio = True
    if save == True:
        feature_names = '_'.join(feature_list)
        directory_name = f"rocket_feature_selection/{model_type}_{pressure_threshold}_{power_threshold}_{feature_names}"
...
empty_feature_list = ['Time of day']
f1_reference = rocket_training_run(empty_feature_list)
f1_reference = 0.5881
...
while True:
    ...
    for feature in candidate_features:
        ...
        f1 = rocket_training_run(current_features)
...
with open('rocket_f1_scores.json', 'w') as file:
